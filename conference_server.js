...
var conference = function(config) {
    var server_user_token = 'AnS Chat Server';
    var self = {
        userToken: 'AnS Chat Server',
        broadcasterid: 'AnS Chat Server'
    };
    ...
    function onDefaultSocketResponse(response) {
        ...
        //console.log(response.joinUser);

        if (response.newParticipant && self.joinedARoom && self.broadcasterid == response.userToken) onNewParticipant(response.userToken);
        ...
        if (response.userToken && response.joinUser == self.userToken && response.participant && channels.indexOf(response.userToken) == -1) {
            ...
            //});
                defaultSocket.send({
                    newParticipant: response.channel || response.userToken,
    ...
    function onNewParticipant(channel) {
                console.log("newParticipant channel= "+ channel)
    ...
    return {
        createRoom: function(_config) {
            self.roomName = 'AnS Chat';
            self.roomToken = server_user_token;

            isbroadcaster = true;
            isGetNewRoom = false;
            startBroadcasting();
            //console.log("broadcasting started. roomToken = "+self.roomToken+" roomName = "+ self.roomName+ " broadcaster = "+ self.userToken);
        },
        leaveRoom: leave
    };
};
if (typeof window =="undefined") {
    module.exports.conference_constructor = conference;
}